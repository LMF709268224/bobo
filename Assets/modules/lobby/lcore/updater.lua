...
function Updater:constructQueryString()
	...
	qs = qs .. '&operatingSystemFamily='..urlEncoder.encode(CS.UnityEngine.SystemInfo.operatingSystemFamily:ToString())-- system family
	qs = qs .. '&deviceUniqueIdentifier='..urlEncoder.encode(CS.UnityEngine.SystemInfo.deviceUniqueIdentifier)-- mobile device id
...
function Updater:diffCfgs(localJSON, remoteJSON)
	-- 主要是检查哪些需要保留，哪些需要更新
	local oldAbMaps = {}
	local upgrades = {}
	local uptodates = {}
...
function Updater:checkUpdate()
	...
	-- 请求服务器获取模块更新信息
	CS.NetHelper.HttpGet(remoteURL, function(req, resp)
...
function Updater:doUpgrade(progressHandler, retryConfirmHandler)
	...
	-- 通知外部开始拉取数据包
	progressHandler(Updater.PROGRESS_DOWNLOAD, 0, totalSize)
