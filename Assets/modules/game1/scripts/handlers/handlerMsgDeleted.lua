--[[
    处理服务器下发的通知各个玩家，房间已经被解散，或者被删除
]]

local Handler={}
Handler.VERSION='1.0'
local dfPath = "GuanZhang/Script/"

local dfCompatibleAPI = require(dfPath ..'dfMahjong/dfCompatibleAPI')

function Handler:onMsg(msgData, room)
    --print(' room deleted msg')
    room.isDestroy = true

    local msgDelete = pkproto2.MsgRoomDelete()
    msgDelete:ParseFromString(msgData)

    local msg = "房间已解散"
    if msgDelete.reason == pkproto2.IdleTimeout then
        msg = "房间空置时间过长，被解散"
    elseif msgDelete.reason == pkproto2.DisbandByOwnerFromRMS then
        msg = "房间被房主解散"
    elseif msgDelete.reason == pkproto2.DisbandByApplication then
        msg = "房间被申请解散"
        return false --房间被申请解散时，这里就不处理了 (直接返回)
    elseif msgDelete.reason == pkproto2.DisbandBySystem then
        msg = "房间被系统解散"
    elseif msgDelete.reason == pkproto2.DisbandMaxHand then
        msg = "房间已达到最大局数，被解散"
        return false --为了在大结算的时候直接返回大厅，这里就不处理了 (直接返回)
    elseif msgDelete.reason == pkproto2.DisbandInLoseProtected then
        msg = "房间已有足够人进园子，牌局被解散"
    end
    dfCompatibleAPI:closeDialog()
    local waitCo = coroutine.running()
    local dialog = room:ShowMessageBoxFromDaFeng(
    msg,2,
    function()
        local flag, msg =  coroutine.resume(waitCo)
        if not flag then
            logError(msg)
			return
        end
    end
    )
    dialog.OnMenuBack = function( ... )
        return false
    end
    coroutine.yield()
end

return Handler
