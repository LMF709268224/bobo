...
#region Properties
        ...
        /// <summary>
        /// Called when a new pong message is received from the server.
        /// </summary>
        public OnWebSocketBinaryDelegate OnPong;
...
#region Request Callbacks
        ...
        private void OnInternalRequestUpgraded(HTTPRequest req, HTTPResponse resp)
        {
            ...
            webSocket.OnPong = (ws, msg) =>
            {
                if (OnPong != null)
                    OnPong(this, msg);
            };

            webSocket.PingDataProvider = ()=>
            {
                byte[] bytes;
                if (PingDataProvider != null)
                {
                    bytes = PingDataProvider();
                }
                else
                {
