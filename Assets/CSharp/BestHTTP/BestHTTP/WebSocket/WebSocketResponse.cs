...
namespace BestHTTP.WebSocket
{
    public sealed class WebSocketResponse : HTTPResponse, IHeartbeat, IProtocol
    {
        ...
        /// <summary>
        /// Called when a Pong message received
        /// </summary>
        public Action<WebSocketResponse, byte[]> OnPong;
        
        /// <summary>
        /// ping data provider, called every time when websocket send ping data
        /// </summary>
        public PingDataProviderDelegate PingDataProvider;
...
#pragma warning restore 4014
        ...
        void IHeartbeat.OnHeartbeatUpdate(TimeSpan dif)
        {
            ...
            if (DateTime.UtcNow - lastPing >= PingFrequnecy)
            {
                try
                {
                    byte[] bytes = PingDataProvider();
                    Send(new WebSocketFrame(this.WebSocket, WebSocketFrameTypes.Ping, bytes));
                }
                catch
                {
                    this.closed = true;
